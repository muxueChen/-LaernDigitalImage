#iChannel0 "file://mipmap0_V.glsl"
#iUniform int mSize_1H = 11 in {3, 50}
...
void main() {
	float sigma = float(mSize_1H)/2.0;
	float Z = normpdf(0.0, sigma);
	vec4 final_colour = Z*texture(iChannel0, gl_FragCoord.xy / iResolution.xy);

	for (int j = 1; j < mSize_1H/2; ++j) {
		float ratio = normpdf(float(j), sigma);
		Z += ratio*2.0;
		final_colour += ratio*getAlpha((gl_FragCoord.xy+vec2(float(j)*2.0, 0.0)) / iResolution.xy);
		final_colour += ratio*getAlpha((gl_FragCoord.xy+vec2(float(-j)*2.0, 0.0)) / iResolution.xy);
	}
